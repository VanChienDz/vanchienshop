...
async function loadExecutorData() {
    try {
        const response = await fetch("./executor.json", {
            cache: "no-store"
        });
        if (!response.ok) {
        ...
        DATA = Array.isArray(json) ? json : (json && Array.isArray(json.executors) ? json.executors : []);
    } catch (error) {
        DATA = [];
        console.warn("executor.json failed to load.", error);
    }
...
function stars(number) {
    ...
    for (let index = 0; index < 5; index += 1) {
        const className = index < full ? "filled" : (index === full && half ? "half" : "empty");
...
function formatVersion(value) {
    ...
    return version.length > 10 ? `${version.slice(0, 7)}...` : version;
}
...
function createCard(item) {
    ...
    <span class="meta-group global" title="Global Version ${escapeHtml(item.version || "—")} · ${escapeHtml(capitalize(item.status))}">
      ...
      <span class="version-chip">${escapeHtml(formatVersion(item.version || "—"))}</span>
    ...
    const vngMeta = hasVng ?
      ...
      <span class="meta-group vng" title="VNG Version ${escapeHtml(item.versionVng || "—")} · ${escapeHtml(capitalize(item.statusVng))}">
        ...
        <span class="version-chip">${escapeHtml(formatVersion(item.versionVng || "—"))}</span>
...
function renderAll() {
    ...
    animateCount(document.getElementById("total-count"), list.length);
    animateCount(document.getElementById("working-count"), list.filter((item) => item.status === "working").length);
    animateCount(document.getElementById("down-count"), list.filter((item) => item.status === "down").length);
    animateCount(document.getElementById("updating-count"), list.filter((item) => item.status === "updating").length);
}
...
document.addEventListener("keydown", (event) => {
    if (event.key === "/" && document.activeElement !== q && !modal.classList.contains("open")) {
